...
<script>
// Componente de Tags (igual ao financeiro)
(function() {
...
async function submitNewFolder() {
    ...
    try {
        const response = await fetch('<?php echo url('/drive/folders'); ?>', {
            method: 'POST',
            body: formData
        });
...
async function toggleFavorite(fileId) {
    try {
        const response = await fetch(`<?php echo url('/drive'); ?>/${fileId}/favorite`, {
            method: 'POST'
        });
...
async function trashFile(fileId) {
    ...
    try {
        const response = await fetch(`<?php echo url('/drive'); ?>/${fileId}/trash`, {
            method: 'POST'
        });
...
async function deleteFolder(folderId) {
    ...
    try {
        const response = await fetch(`<?php echo url('/drive/folders'); ?>/${folderId}`, {
            method: 'DELETE'
        });
...
/* Select2 dentro de modal - apenas z-index */
.select2-dropdown {
